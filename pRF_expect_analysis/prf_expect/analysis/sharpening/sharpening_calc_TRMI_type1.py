...
output_dir = os.path.join(
    data_dir, "derivatives", "prf_data", subject, "ses-1", f"TRMI-type1_sharpening-{broaden_factor_fn}"
)
...
for run in PE_runs:
    ...
    spar_pred_run = np.load(
        os.path.join(
            pred_dir, f"{subject}_ses-1_task-sparse_{run}_space-fsaverage_preds_sharpening-{broaden_factor_fn}.npy"
        )
    ).T
