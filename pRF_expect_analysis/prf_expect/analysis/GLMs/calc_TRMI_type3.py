"""
GLM analysis type 3: Sparse and omission regressor amplitudes were estimated separately from
sparse- and omission-condition data, respectively
"""
...
def fit_viol_model(
    ...
    residuals0 = viol_data - betas_omit * omit_pred - betas_spar * spar_pred
    nan_nr = 0
    for vertex in range(residuals0.shape[1]):
        ...
        x_omit_spar = np.vstack(
            [
                spar_pred[:, vertex],
                omit_pred[:, vertex],
            ]
        ).T
        betas_omit_spar = np.array([betas_omit[vertex], betas_spar[vertex]])
        t_contrast[vertex], p_contrast[vertex] = calc_t_stat(
            x_omit_spar,
            viol_data[:, vertex],
            np.dot(x_omit_spar, betas_omit_spar),
            betas_omit_spar,
            [-1, 1],
        )

        rsq[vertex] = 1 - np.sum(residuals0[:, vertex] ** 2) / np.sum(
...
def fit_omit_model(omit_data, pRF_pred, omit_pred):
    ...
    rsq = con_nan_1darray(omit_data)
    nan_nr = 0
    for vertex in range(omit_data.shape[1]):
        if (
            np.isnan(omit_pred[:, vertex]).any()
            or np.isnan(prf_expect[:, vertex]).any()
        ):
            nan_nr += 1
            continue
        ...
        rsq[vertex] = 1 - np.sum(residuals1 ** 2) / np.sum(
            (omit_data[:, vertex] - np.nanmean(omit_data[:, vertex])) ** 2
        )
...
output_dir = os.path.join(
    data_dir, "derivatives", "prf_data", subject, "ses-1", "TRMI-type3"
)
...
omit_data = omission_pred * x0 + sparse_hrf_convolve * x1 + prfexpect * x3 + residuals
"""

o_betas_omit, o_rsq = (
    fit_omit_model(omit_data, pRF_pred, omit_pred)
)
...
np.save(
    os.path.join(
        output_dir,
        f"{subject}_ses-1_space-fsaverage_surprise-omit_rsq.npy",
    ),
    o_rsq,
)
...
"""
loop over vertices for
spar_data = spar_pred * x0 + residuals
"""
